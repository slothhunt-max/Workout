...
$repl8 = @'
    const targetRepsInput = container.querySelector('#schedule-target-reps');
    const targetWeightInput = container.querySelector('#schedule-target-weight');
    if (targetRepsInput) targetRepsInput.value = '';
...
$repl10 = @'
  btnAdd.addEventListener('click', () => {
    ...
    if (exerciseId) {
      if (editModeItemId) {
        if (confirm('수정하시겠습니까?')) {
          store.updateRoutineItem(currentRoutineId, editModeItemId, '10', '');
          resetScheduleForm();
        }
      } else {
...
if ($startIndex -eq -1 -or $endIndex -eq -1) { Write-Output "Not found"; exit }
...
$finalCode = $before + $newWorkoutTab + "`n" + $after
Set-Content .\js\app.js $finalCode -Encoding UTF8

# Remove Timer tab entirely
$tStartIndex = $finalCode.IndexOf('// --- Timer Tab ---')
$tEndIndex = $finalCode.IndexOf('// --- Exercises Tab ---')
if ($tStartIndex -ne -1 -and $tEndIndex -ne -1) {
    $beforeT = $finalCode.Substring(0, $tStartIndex)
    $afterT = $finalCode.Substring($tEndIndex)
    Set-Content .\js\app.js ($beforeT + $afterT) -Encoding UTF8
}

Write-Output "All fixes applied using Single-Quoted strings!"
