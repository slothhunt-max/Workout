    ...
    if (currentView === 'list') {
      ...
      let listHtml = `
        <div style="display: flex; align-items: center; justify-content: center; margin-bottom: 16px; position: relative; min-height: 40px;">
          <button id="btn-export-csv" class="btn btn-secondary" style="position: absolute; left: 0; width: auto; padding: 6px 12px; font-size: 0.8rem; margin: 0;">내보내기</button>
          <h1 class="page-title" style="margin:0; text-align: center;">기록 관리</h1>
          <label class="btn btn-primary" style="position: absolute; right: 0; margin: 0; width: auto; padding: 6px 12px; font-size: 0.8rem; cursor: pointer;">
            불러오기
      ...
      if (data.length === 0) {
            ...
            <table style="width: max-content; border-collapse: separate; border-spacing: 0; text-align: center; font-size: 0.85rem;">
              <thead>
                <tr>
                  ...
                  ${data.map(d => `<th class="col-header date-header" data-raw-date="${d.date}" style="padding: 12px; border-right: 3px solid var(--border-color); min-width: 80px; background: var(--bg-color); border-bottom: 1px solid var(--border-color); cursor: pointer;">${d.date.substring(2)}</th>`).join('')}
                </tr>
                <tr>
        ...
        const labels = data.map(d => d.date.substring(2)); // YY/MM/DD
        ...
        if (tableContainer && chartInstance) {
          tableContainer.addEventListener('click', async (e) => {
            const headerCell = e.target.closest('.date-header');
            if (headerCell) {
              const rawDate = headerCell.dataset.rawDate;
              const action = await window.showActionSheet('Manage Record: ' + rawDate, [
                { text: '수정', value: 'edit', style: 'primary' },
                { text: '삭제', value: 'delete', style: 'destructive' }
              ]);
              
              if (action === 'delete') {
                if (confirm(rawDate + ' 기록을 삭제하시겠습니까?')) {
                  store.state.history.forEach(h => {
                    let dObj = new Date(h.date.replace(/[\.\-\/]/g, '/'));
                    let dStr = isNaN(dObj.getTime()) ? h.date : `${dObj.getFullYear()}/${String(dObj.getMonth() + 1).padStart(2, '0')}/${String(dObj.getDate()).padStart(2, '0')}`;
                    if (dStr === rawDate) {
                      if (h.records) {
                        h.records = h.records.filter(r => {
                          const exName = h.exercisesSnapshot?.find(ex => ex.id === r.exerciseId)?.name;
                          return getMappedName(exName) !== selectedExerciseGroup;
                        });
                      }
                      if (h.importedRecords) {
                        h.importedRecords = h.importedRecords.filter(r => getMappedName(r.name) !== selectedExerciseGroup);
                      }
                    }
                  });
                  store.state.history = store.state.history.filter(h => (h.records && h.records.length > 0) || (h.importedRecords && h.importedRecords.length > 0));
                  store.save('history');
                  renderContent();
                }
              } else if (action === 'edit') {
                const d = data.find(item => item.date === rawDate);
                if (d && d.sets) {
                  const newSets = await window.showEditSetsModal('수정: ' + rawDate, d.sets);
                  if (newSets) {
                    d.sets.forEach((s, idx) => {
                      if (newSets[idx]) {
                        s.weight = newSets[idx].weight;
                        s.reps = newSets[idx].reps;
                      }
                    });
                    store.save('history');
                    renderContent();
